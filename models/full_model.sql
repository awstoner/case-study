...
WITH churn_return_calc AS (
    SELECT
        ...
        , customer_flag
        , CASE
            WHEN LAG(customer_flag, 1) OVER (PARTITION BY user_id ORDER BY month) = 1 AND customer_flag = 0 THEN TRUE
            ELSE FALSE
        END AS churn_flag
        , CASE
            WHEN LAG(customer_flag, 1) OVER (PARTITION BY user_id ORDER BY month) = 0 AND customer_flag = 1 THEN TRUE
            ELSE FALSE
        END AS return_flag
    FROM splice_case_study.incremental_model
    WHERE DATE_DIFF('month', cohort_month, month) >= 0
    AND NOT (cohort_month = month AND customer_flag = 0)
